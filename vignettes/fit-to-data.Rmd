...
l <- array(c(0.4, 0.7, 0.5), dim = c(1,3,1))
...
f <- mmmRates(f_long, cols, reps, lims, inst = TRUE)
#
f <- array(f, dim = c(nrow(f), ncol(f), 1))
...
w <- mmmWeights(tags)
#
w <- array(w, dim = c(nrow(w), ncol(w), 1))
